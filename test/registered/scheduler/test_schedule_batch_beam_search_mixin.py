...
class TestPrepareForBeamSearchDecode(unittest.TestCase):
    ...
    def test_prepare_for_beam_search_decode_different_beam_widths(
        ...
        req1.kv = SimpleNamespace(kv_allocated_len=5)

        req2 = Mock()
        req2.beam_width = 3
        req2.beam_list = Mock()
        req2.beam_list.last_tokens = torch.tensor([300, 400, 500], device=self.device)
        req2.kv = SimpleNamespace(kv_allocated_len=6)
        ...
        def mock_alloc(batch, token_per_req):
            # Mirror main's alloc_for_decode bookkeeping side effect.
            for req in batch.reqs:
                req.kv.kv_allocated_len += token_per_req
            return torch.tensor([10, 11, 12, 13, 14], device=self.device)

        mock_alloc_for_decode.side_effect = mock_alloc
        ...
        self.assertEqual(req1.kv.kv_allocated_len, 5)
        self.assertEqual(req2.kv.kv_allocated_len, 6)
        self.assertTrue(
            torch.equal(
                batch.out_cache_loc,
