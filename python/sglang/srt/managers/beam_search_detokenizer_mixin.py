...
class BeamSearchDetokenizerMixin:
    ...
    def decode_beam_search_output(self, recv_obj: BatchTokenIDOutput):
        ...
        nvtx.range_push("beam_search:detokenize")
        if self.disable_tokenizer_batch_decode:
            ...
            nvtx.range_push("beam_search:detokenize:trim")
            beam_ids = []
            for i, beam_output in enumerate(recv_obj.beam_search_output):
                for beam in beam_output.sequences:
                    beam_ids.append(
            ...
            nvtx.range_pop()  # beam_search:detokenize:trim

            nvtx.range_push("beam_search:detokenize:batch_decode")
            beam_texts = self.tokenizer.batch_decode(
                beam_ids,
                skip_special_tokens=recv_obj.skip_special_tokens[0],
                spaces_between_special_tokens=recv_obj.spaces_between_special_tokens[0],
            )
            nvtx.range_pop()  # beam_search:detokenize:batch_decode

            nvtx.range_push("beam_search:detokenize:batch_assign")
            i = 0
            for beam_output in recv_obj.beam_search_output:
                for beam in beam_output.sequences:
                    ...
                    i += 1
            nvtx.range_pop()
        nvtx.range_pop()  # beam_search:detokenize
