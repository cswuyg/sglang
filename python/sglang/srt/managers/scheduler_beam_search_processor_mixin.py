...
class SchedulerBeamSearchProcessorMixin:
    ...
    def process_beam_search_prefill_result(
        ...
        """
        nvtx.range_push("beam_search:prefill_result")
        ...
        for i, req in enumerate(batch.reqs):
            ...
            # Apply constraint mask on logprobs if constraint is configured
            if self.beam_search_constraint is not None:
                nvtx.range_push("beam_search:apply_prefill")
                self.beam_search_constraint.apply_prefill(logprobs_i)
                nvtx.range_pop()
        ...
        # Handle penalty state expansion after processing all requests


        # Handle penalty state expansion after processing all requests
        self.handle_prefill_penalty_expansion(batch, logits_output.logprobs.device)
    ...
    def process_beam_search_decode_result(
        ...
        """
        nvtx.range_push("beam_search:decode_result")
        self.num_generated_tokens += len(batch.req_pool_indices)

        if getattr(self, "enable_beam_mamba_double_buffer", False) and isinstance(
            self.req_to_token_pool, HybridReqToTokenPool
        ):
            self.req_to_token_pool.flush_beam_cow_source_slots()
        ...
        if (
            ...
            self.report_decode_stats(result.can_run_cuda_graph, running_batch=batch)
        nvtx.range_pop()
    ...
    def _create_completed_beams_for_insufficient_candidates(
        ...
        if req.finished_reason is None:
            # completed[0] 的 finish_reason 可能为 None（top-1 候选未完成，
            # 但未完成候选总数 < beam_width）。这里取第一个非空的 finish_reason，
            # 找不到则兜底为 FINISH_LENGTH，保证请求结束原因一定被正确标记。
            first_reason = next(
                (b.finish_reason for b in completed if b.finish_reason is not None),
                None,
            )
            req.finished_reason = (
                first_reason if first_reason is not None else FINISH_LENGTH(length=1)
            )
    ...
    def _create_initial_beam_sequences(
        ...
        for i, (top_logprob, top_token) in enumerate(
            ...
            if is_finished:
                ...
                # 为前缀约束 beam search 初始化 trie_node
                if self.beam_search_constraint is not None:
                    nvtx.range_push("beam_search:init_beam_node")
                    self.beam_search_constraint.init_beam_node(beam_sequence, top_token)
                    nvtx.range_pop()
        ...
        # 当约束导致合法候选数少于 beam_width 时，用 dummy beam 填充，
        # 以保证 KV cache 分配正确（需要 beam_width 个 slot）。
        has_dummy = False
        if len(incomplete_beams) < req.beam_width and incomplete_beams:
            if self.beam_search_constraint is not None:
                has_dummy = True
                while len(incomplete_beams) < req.beam_width:
                    last_valid = incomplete_beams[-1]
                    dup_beam = BeamSearchSequence(
                        tokens=list(last_valid.tokens),
                        cum_logprob=last_valid.cum_logprob,
                        trie_node=last_valid.trie_node,
                        is_dummy=True,
                    )
                    incomplete_beams.append(dup_beam)
                    cum_logprobs.append(last_valid.cum_logprob)
                    last_token_ids.append(last_valid.tokens[-1])

        req.beam_list.completed = completed_beams
        req.beam_list.incomplete = incomplete_beams
        req.beam_list.has_dummy = has_dummy
    ...
    def _extract_beam_topk_data(
        ...
        """
        nvtx.range_push("beam_search:extract_topk")

        # 若配置了约束，则对 logprobs 施加约束 mask。
        # retracted 请求在 retract_decode 时已被 filter_batch 移出 batch.reqs，
        # 故 ordered_beams 与 logprobs 的行天然对齐。
        if self.beam_search_constraint is not None:
            nvtx.range_push("beam_search:apply_constraint")
            ordered_beams = [b for req in batch.reqs for b in req.beam_list.incomplete]
            self.beam_search_constraint.apply_decode(
                result.logits_output.logprobs, ordered_beams
            )
            nvtx.range_pop()
    ...
    def _expand_and_prune_beams(
        ...
        # -inf 候选仅可能来自 dummy beam 或约束 mask；
        # 跳过它们，避免污染有效的 beam 名额。
        skip_inf = req.beam_list.has_dummy or (
            self.beam_search_constraint is not None
        )
        ...
        if not has_stop_strs and (ignore_eos or not req.stop_token_ids):
            for last_beam_idx, top_token, cum_logprob in zip(
                ...
                if skip_inf and cum_logprob == float("-inf"):
                    continue
        ...
        # 为所有新 beam 维护前缀约束 beam search 的 trie_node。
        # 顺序敏感：必须在下方 `req.beam_list.incomplete` 被重新赋值之前、
        # 以及 dummy 填充之前执行，因为 keep_last_beam_indices 是针对
        # 旧 incomplete 列表（req.beam_list.incomplete）的索引。
        if self.beam_search_constraint is not None:
            nvtx.range_push("beam_search:step_beam_node")
            old_incomplete = req.beam_list.incomplete
            for new_beam, last_beam_idx in zip(incomplete, keep_last_beam_indices):
                parent_beam = old_incomplete[last_beam_idx]
                self.beam_search_constraint.step_beam_node(
                    new_beam, parent_beam, new_beam.tokens[-1]
                )
            nvtx.range_pop()
        ...
        if len(incomplete) < beam_width:
            # completed 已达 beam_width，或没有 incomplete 可继续 → 正常结束
            if len(req.beam_list.completed) >= beam_width or len(incomplete) == 0:
                ...
                nvtx.range_pop()
                return None
            # 用 dummy beam 填充至 beam_width，保证 KV cache 分配正确
            template = incomplete[0]
            template_idx = keep_last_beam_indices[0]
            while len(incomplete) < beam_width:
                dummy = BeamSearchSequence(
                    tokens=list(template.tokens),
                    cum_logprob=template.cum_logprob,
                    trie_node=template.trie_node,
                    is_dummy=True,
                )
                incomplete.append(dummy)
                keep_last_beam_indices.append(template_idx)
                last_token_ids.append(last_token_ids[0])
                incomplete_cum_logprobs.append(template.cum_logprob)
            req.beam_list.has_dummy = True
    ...
    def _create_completed_beams_for_finished_request(
        ...
        # 过滤掉 -inf 候选（仅可能来自 dummy beam 或约束 mask）。
        # 继续推理路径（_expand_and_prune_beams）在扩展分支时会显式 continue 跳过 -inf，
        # 但此处是"结束请求、直接把 topk 选为完成序列"的路径，topk[:beam_width]
        # 在可用候选不足时仍可能把 -inf 选进来，因此必须在这里再过滤一次。
        skip_inf = req.beam_list.has_dummy or (
            self.beam_search_constraint is not None
        )
        if skip_inf:
            # -inf 是极少见的边界情况，且降序保证它连续落在尾部，
            # 故从尾部反向扫描，效率更高。
            n_valid = len(topk_values_cpu)
            while n_valid > 0 and topk_values_cpu[n_valid - 1] == float("-inf"):
                n_valid -= 1
            if n_valid != len(topk_values_cpu):
                last_beam_indices_cpu = last_beam_indices_cpu[:n_valid]
                top_tokens_cpu = top_tokens_cpu[:n_valid]
                topk_values_cpu = topk_values_cpu[:n_valid]
    ...
    def _handle_beam_kv_cache(
        ...
        3. For hybrid SSM models, remap the recurrent SSM/conv state with the same
           surviving-parent -> new-beam mapping as the attention KV (see
           ``_remap_beam_mamba_states``). This is required because the mamba state is
           an in-place recurrent state per slot, not a token-indexed cache.
        4. Free KV cache indices that are no longer needed (using unique counts, indices appearing only once are pruned)
        ...
        """
        nvtx.range_push("beam_search:kv_cache")
        ...
        if isinstance(self.req_to_token_pool, HybridReqToTokenPool):
            self._remap_beam_mamba_states(src_pool_indices, beam_req_pool_indices)
    ...
    def _remap_beam_mamba_states(
        self: Scheduler,
        src_pool_indices: torch.Tensor,
        dst_pool_indices: torch.Tensor,
    ):
        """Prune mamba states: double-buffer / COW if enabled, otherwise physical copy (aliasing-safe via advanced indexing)."""
        if getattr(self, "enable_beam_mamba_double_buffer", False):
            self._remap_beam_mamba_states_double_buffer(
                src_pool_indices, dst_pool_indices
            )
            return

        nvtx.range_push("beam_search:mamba_state")
        mamba_map = self.req_to_token_pool.req_index_to_mamba_index_mapping
        src_mamba_idx = mamba_map[src_pool_indices].long()
        dst_mamba_idx = mamba_map[dst_pool_indices].long()

        # Mamba state is large. A single advanced-index copy can allocate a
        # multi-GB temporary for high beam widths, so snapshot each unique source
        # slot to CPU first and restore destinations one by one. This preserves
        # correctness when source and destination slots overlap, while keeping
        # peak GPU temporary memory bounded by one mamba slot.
        physical_src = self.req_to_token_pool.translate_mamba_indices(src_mamba_idx)
        physical_dst = self.req_to_token_pool.translate_mamba_indices(dst_mamba_idx)
        snapshots = {}
        for src in torch.unique(physical_src).tolist():
            src_tensor = torch.tensor([src], dtype=torch.long, device=physical_src.device)
            snapshots[int(src)] = self.req_to_token_pool.mamba_pool.get_cpu_copy(src_tensor)

        for src, dst in zip(physical_src.tolist(), physical_dst.tolist()):
            if src == dst:
                continue
            dst_tensor = torch.tensor([dst], dtype=torch.long, device=physical_dst.device)
            self.req_to_token_pool.mamba_pool.load_cpu_copy(snapshots[int(src)], dst_tensor)

        if self.req_to_token_pool.mamba_pool.replayssm_write_pos is not None:
            self.req_to_token_pool.mamba_pool.replayssm_write_pos[physical_dst] = 0
        nvtx.range_pop()

    def _remap_beam_mamba_states_double_buffer(
        self: Scheduler,
        src_pool_indices: torch.Tensor,
        dst_pool_indices: torch.Tensor,
    ):
        """Zero-copy prune via double buffering: alloc fresh slots, set COW read mapping, stage old slots for delayed free."""
        nvtx.range_push("beam_search:mamba_state_double_buffer")
        pool = self.req_to_token_pool
        mapping_dtype = pool.req_index_to_mamba_index_mapping.dtype

        src_mamba_idx = pool.req_index_to_mamba_index_mapping[src_pool_indices]
        pool.set_beam_cow_read_mapping(
            dst_pool_indices, src_mamba_idx.to(dtype=mapping_dtype)
        )

        old_dst_mamba_idx = pool.req_index_to_mamba_index_mapping[dst_pool_indices]

        total = dst_pool_indices.numel()
        new_dst_mamba_idx = pool.mamba_allocator.alloc(total)
        if new_dst_mamba_idx is None:
            raise RuntimeError(
                "Out of mamba cache space for beam search double buffer. Please "
                "increase --max-mamba-cache-size / --mamba-full-memory-ratio, or "
                "reduce --max-running-requests / beam width, or disable "
                "--enable-beam-mamba-double-buffer."
            )
        new_dst_mamba_idx = new_dst_mamba_idx.to(
            device=src_mamba_idx.device, dtype=mapping_dtype
        )
        pool.req_index_to_mamba_index_mapping[dst_pool_indices] = new_dst_mamba_idx

        pool.stage_beam_cow_source_slots(old_dst_mamba_idx)
        nvtx.range_pop()
    ...
    def _cache_finished_beam_search(self: Scheduler, batch: ScheduleBatch):
        ...
        # Mapping entries are dead data after free, overwritten on next alloc before any read.
        if isinstance(self.req_to_token_pool, HybridReqToTokenPool):
            beam_mamba_idx = self.req_to_token_pool.req_index_to_mamba_index_mapping[
                beam_pool_indices
            ].long()
            self.req_to_token_pool.mamba_allocator.free(beam_mamba_idx)
    ...
    # ─── Penalty State Management for Beam Search ─────────────────────────


    def handle_prefill_penalty_expansion(
        ...
        Args:
            batch: Schedule batch containing beam search requests
            device: Device where tensors are located
        Note: 必须保持 penalty 为完整的形状，等到 filter_batch 的时候再做完成请求的过滤
        """
        if (
        ...
        nvtx.range_push("beam_search:penalty_prefill")
        ...
        for req_idx, req in enumerate(batch.reqs):
            if req.beam_list.last_tokens is None:
                # 当请求结束的时候，last_tokens 为 None，需要做补丁
    ...
    def handle_decode_penalty_expansion(
        self,
        batch: "ScheduleBatch",
        reqs_for_kv_copy: List["Req"],
        last_batch_slot_indices_list: List[torch.Tensor],
    ) -> None:
        ...
        1. Reorders penalty states to match the new beam structure (after expansion/pruning)
        2. Cumulates the newly generated tokens into penalty states

        Args:
            batch: Schedule batch containing beam search requests
            reqs_for_kv_copy: List of requests that have beam reordering
        ...
        nvtx.range_push("beam_search:penalty_decode")

        req_to_kv_copy_idx = {req: idx for idx, req in enumerate(reqs_for_kv_copy)}
        ...
        for req in batch.reqs:
            if req.finished():
                ...
                req_idx_in_list = req_to_kv_copy_idx.get(req)

                if req_idx_in_list is not None:
                    # This request has beam reordering
                    last_batch_slot_indices = last_batch_slot_indices_list[
                        req_idx_in_list
                    ]
                    base_idx = req.beam_list.batch_slot_start_idx
                    # Convert batch slot indices to relative indices within current penalty state
                    relative_indices = (
                        last_batch_slot_indices - base_idx + current_beam_offset
                    ).tolist()
                    global_reorder_indices.extend(relative_indices)
                else:
                    # No reordering for this request, keep original order
                    global_reorder_indices.extend(
                        range(
                            current_beam_offset,
